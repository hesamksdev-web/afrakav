            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-[10px] text-muted-foreground border-b border-border">
                  ...
                  <th className="text-end px-4 py-2 font-normal">اقدامات</th>
                </tr>
              </thead>
              <tbody>
                {customers.map(c => (
                  <tr key={c.id} className="border-b border-border last:border-0 hover:bg-secondary/20">
                    ...
                    <td className="px-4 py-2.5">
                      <CustomerActions customer={c} onChanged={refresh} />
                    </td>
                  </tr>
                ))}
              </tbody>
  ...
  return (
    <div className="bg-card border border-border rounded p-4">
      ...
      <form onSubmit={submit} className="space-y-2.5">
        ...
        <Field label="رمز عبور" value={password} onChange={setPassword} placeholder="حداقل ۱۲ نویسه" type="password" ltr />
        <Msg msg={msg} />
...
// Suspending an account and resetting its password both void that customer's
// outstanding tokens, so either one signs them out immediately.
function CustomerActions({ customer, onChanged }: { customer: Customer; onChanged: () => void }) {
  ...
  const toggle = async () => {
    const suspending = !customer.disabled;
    const verb = suspending ? "غیرفعال" : "فعال";
    if (!confirm(`حساب «${customer.username}» ${verb} شود؟ نشست‌های باز این مشتری بسته می‌شوند.`)) return;
    setBusy(true); setMsg(null);
    try {
      await setCustomerStatus(customer.id, suspending);
      onChanged();
    } catch (err: any) {
      setMsg({ ok: false, text: err?.message || "به‌روزرسانی حساب ناموفق بود" });
    } finally { setBusy(false); }
  };
  ...
  const reset = async (e: React.FormEvent) => {
    ...
    setBusy(true); setMsg(null);
    try {
      await resetCustomerPassword(customer.id, password);
  ...
  return (
    <div className="flex flex-col items-end gap-1.5">
      <div className="flex items-center gap-2">
        <button type="button" onClick={() => { setResetting(v => !v); setMsg(null); }} disabled={busy}
          title="تغییر رمز عبور مشتری"
        ...
        <button type="button" onClick={toggle} disabled={busy}
          ...
          className={`transition-colors disabled:opacity-40 ${
            customer.disabled ? "text-muted-foreground hover:text-primary" : "text-muted-foreground hover:text-[#ff3b3b]"
          }`}>
          {busy ? <Loader2 size={13} className="animate-spin" /> : customer.disabled ? <Undo2 size={13} /> : <Ban size={13} />}
        </button>
      </div>
